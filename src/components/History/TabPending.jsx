...
const TabPending = () => {
  ...
  useEffect(() => {
    fetch("http://localhost/DACS_Hutech/backend/get_orders.php?status=Chờ xác nhận")
      .then((res) => res.json())
      .then((data) => {
        setOrders(data);
        setLoading(false);
  ...
  return (
    <div className="space-y-4">
      {orders.map((order) => (
        <div
          key={order.id}
          className="p-4 border rounded-lg shadow-sm bg-yellow-50"
        >
          <div className="font-medium text-[#b56c00]">
