...
const DeliveryInfo = ({ selectedVehicle, pickupAddress, deliveryAddress, distance }) => {
    ...
    // Pickup Info
    const [pickupAddressState, setPickupAddressState] = useState(pickupAddress || "");
    const [pickupDetail, setPickupDetail] = useState("");
    const [senderName, setSenderName] = useState("");
    const [senderPhone, setSenderPhone] = useState("");

    // Delivery Info
    const [deliveryAddressState, setDeliveryAddressState] = useState(deliveryAddress || "");
    ...
    const confirmOrder = async () => {
        ...
        if (paymentMethod === 'balance') {
            try {
                ...
                const balanceResponse = await fetch(`http://localhost/DACS_Hutech/backend/get_balance.php?user_id=${userId}`);
                const balanceData = await balanceResponse.json();

                if (!balanceData.success) {
                    toast.error("Không thể kiểm tra số dư tài khoản. Vui lòng thử lại sau.", {
                        position: "top-right",
                        autoClose: 3000,
                        hideProgressBar: false,
                        closeOnClick: true,
                ...
                if (balanceData.balance < shippingFee) {
                    toast.error("Số dư tài khoản không đủ để thanh toán. Vui lòng nạp thêm tiền hoặc chọn phương thức thanh toán khác.", {
                        position: "top-right",
                        autoClose: 5000,
                        hideProgressBar: false,
                        closeOnClick: true,
                ...
                // Trừ tiền từ ví
                const deductResponse = await fetch("http://localhost/DACS_Hutech/backend/wallet_handler.php", {
                    method: "POST",
        ...
        const calculateShippingFee = () => {
            if (selectedVehicle && distance) {
                const pricePerKm = 
                    ...
                    selectedVehicle === 'Xe Tải' ? 20000 : 5000;
        ...
        <div className="w-full mt-10 pr-6">
            ...
            <div className="mb-8 bg-white py-6 rounded-2xl px-6 shadow-lg border border-gray-200">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <p className="font-semibold text-[16px] text-[#4e7cb2]">Địa điểm lấy hàng</p>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <div className="relative">
                        <input 
                            ...
                            className={`rounded-lg p-3 w-full bg-gray-50 border ${errors.pickupAddress ? 'border-red-500' : 'border-gray-300'} focus:border-[#4e7cb2] focus:ring-1 focus:ring-[#4e7cb2] outline-none transition-all`}
                            required
                        />
                   ...
                   <input 
                        ...
                        className={`rounded-lg p-3 w-full bg-gray-50 border ${errors.senderName ? 'border-red-500' : 'border-gray-300'} focus:border-[#4e7cb2] focus:ring-1 focus:ring-[#4e7cb2] outline-none transition-all`}
                        required 
                    />
                    {errors.senderName && (
                            <p className="text-red-500 text-sm mt-1">Vui lòng nhập tên người gửi</p>
                        )}
                   </div>
                    <div>
                        <input 
                            ...
                            className={`rounded-lg p-3 w-full bg-gray-50 border ${errors.senderPhone ? 'border-red-500' : 'border-gray-300'} focus:border-[#4e7cb2] focus:ring-1 focus:ring-[#4e7cb2] outline-none transition-all`}
                            required 
                        />
                          {errors.senderPhone && (
            ...
            <div className="mb-8 bg-white py-6 rounded-2xl px-6 shadow-lg border border-gray-200">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <p className="font-semibold text-[16px] text-[#4e7cb2]">Bạn muốn giao hàng đến</p>
                </div>
                <div className="grid grid-cols-2 gap-4 relative">
                    <div>
                        <input 
                            ...
                            className={`rounded-lg p-3 w-full bg-gray-50 border ${errors.deliveryAddress ? 'border-red-500' : 'border-gray-300'} focus:border-[#4e7cb2] focus:ring-1 focus:ring-[#4e7cb2] outline-none transition-all`}
                            required
                        />
                    ...
                    <div>
                        ...
                        />
                        {errors.receiverName && (
                            <p className="text-red-500 text-sm mt-1">Vui lòng nhập tên người nhận</p>
                        )}
                    </div>
                    <div>
                        <input 
                    ...
                    <div className="relative">
                        <div>
                            <input 
                                ...
                                className={`rounded-lg p-3 w-full bg-gray-50 border ${valueError ? 'border-red-500' : 'border-gray-300'} focus:border-[#4e7cb2] focus:ring-1 focus:ring-[#4e7cb2] outline-none transition-all`}
                            />
                            {errors.goodsValue && (
                                <p className="text-red-500 text-sm mt-1">Vui lòng nhập giá trị hàng</p>
                            )}
                        </div>
                        {valueError && (
            ...
            <div className="mb-8 bg-white py-6 rounded-2xl px-6 shadow-lg border border-gray-200">
                <div className="flex items-center gap-2 mb-6">
                    ...
                    <p className="font-semibold text-[16px] text-[#4e7cb2]">Phương thức thanh toán</p>
                </div>
                    ...
                    <div className="bg-white rounded-2xl w-[500px] max-h-[90vh] overflow-y-auto scrollbar-hide">
                        ...
                        <div className="bg-[#4e7cb2] px-6 py-4 rounded-t-2xl">
                            <div className="flex items-center justify-between">
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-lg font-semibold text-white">Hóa đơn vận chuyển</h3>
                                </div>
                            ...
                            <p className="text-white/80 text-sm mt-2">Mã đơn: #{Math.random().toString(36).substr(2, 9).toUpperCase()}</p>
                        </div>
                            ...
                            <div className="space-y-4">
                                <div className="flex items-center justify-between pb-2 border-b border-dashed border-gray-200">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-medium text-gray-800">Chi tiết vận chuyển</span>
                                    </div>
                                ...
                                <div className="space-y-4">
                                    <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-gray-800">Điểm lấy hàng</p>
                                                <p className="text-gray-600">{pickupAddressState}</p>
                                                {pickupDetail && <p className="text-sm text-gray-500">{pickupDetail}</p>}
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-gray-800">Điểm giao hàng</p>
                                                <p className="text-gray-600">{deliveryAddressState}</p>
                                                {deliveryDetail && <p className="text-sm text-gray-500">{deliveryDetail}</p>}
                                                <div className="mt-2 text-sm">
                                                    <p className="font-medium text-gray-700">{receiverName}</p>
                            ...
                            <div className="space-y-3 pt-4">
                                <div className="flex items-center justify-between pb-2 border-b border-dashed border-gray-200">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-medium text-gray-800">Thông tin hàng hóa</span>
                                    </div>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                                    <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                                        <span className="text-gray-600">Loại hàng</span>
                                        <span className="font-medium text-gray-800">{selectedType}</span>
                                    </div>
                                    <div className="flex justify-between items-center">
                                        <span className="text-gray-600">Giá trị hàng</span>
                                        <span className="font-medium text-gray-800">{parseInt(goodsValue).toLocaleString()} VNĐ</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-3 pt-4">
                                <div className="flex items-center justify-between pb-2 border-b border-dashed border-gray-200">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="font-medium text-gray-800">Chi phí & Thanh toán</span>
                                    </div>
                                </div>
                                <div className="bg-gray-50 p-4 rounded-lg space-y-3">
                                    <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                                        ...
                                        <div className="text-right">
                                            <span className="font-medium text-gray-800">{shippingFee.toLocaleString()} VNĐ</span>
                                    ...
                                    <div className="flex justify-between items-center pt-2 border-t border-gray-200">
                                        ...
                                        <span className="font-medium text-gray-800">
                                            {paymentMethod === 'cod' ? 'Thanh toán khi nhận hàng' : 'Thanh toán qua ví'}
                                        </span>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="bg-gray-50 px-6 py-4 rounded-b-2xl border-t border-gray-200">
                            <div className="flex justify-between items-center mb-4">
                                ...
                                <div className="text-right">
                                    <span className="text-2xl font-bold text-[#4e7cb2]">{shippingFee.toLocaleString()} VNĐ</span>
