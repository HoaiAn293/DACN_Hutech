<?php
session_start();
require_once 'database.php';
header("Access-Control-Allow-Origin: *");
header("Access-Control-Allow-Methods: GET, OPTIONS");
header("Access-Control-Allow-Headers: Content-Type");
header("Content-Type: application/json");

// Xử lý OPTIONS request
if ($_SERVER['REQUEST_METHOD'] === 'OPTIONS') {
    http_response_code(200);
    exit();
}

try {
    // --- CẤU HÌNH GIẢ LẬP DI CHUYỂN ---
    // Hàm tạo độ lệch ngẫu nhiên (tương đương xe di chuyển khoảng 10-50m)
    function getRandomOffset() {
        return (rand(-100, 100)) / 40000;
    }

    // 1. Lấy các tài xế đang active và ĐÃ CÓ VỊ TRÍ
    $sql = "SELECT id, full_name, phone_number, current_lat, current_lng 
            FROM users 
            WHERE role = 'driver' 
            AND status = 'active' 
            AND current_lat IS NOT NULL 
            AND current_lng IS NOT NULL
            AND current_lat != 0 
            AND current_lng != 0";

    $result = $conn->query($sql);
    $drivers = [];

    if ($result && $result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            $driverId = (int)$row['id'];
            $currentLat = (float)$row['current_lat'];
            $currentLng = (float)$row['current_lng'];

            // Kiểm tra vị trí hợp lệ
            if ($currentLat == 0 || $currentLng == 0 || 
                $currentLat < -90 || $currentLat > 90 ||
                $currentLng < -180 || $currentLng > 180) {
                continue; // Bỏ qua driver có vị trí không hợp lệ
            }

            // 2. TÍNH TOÁN VỊ TRÍ MỚI (Giả lập di chuyển)
            // Cộng thêm một khoảng nhỏ vào vĩ độ/kinh độ
            $newLat = $currentLat + getRandomOffset();
            $newLng = $currentLng + getRandomOffset();

            // Đảm bảo vị trí mới vẫn hợp lệ
            if ($newLat < -90 || $newLat > 90 || $newLng < -180 || $newLng > 180) {
                $newLat = $currentLat;
                $newLng = $currentLng;
            }

            // 3. Cập nhật ngay vào Database (để hiển thị realtime)
            $updateSql = "UPDATE users SET current_lat = ?, current_lng = ? WHERE id = ?";
            $updateStmt = $conn->prepare($updateSql);
            if ($updateStmt) {
                $updateStmt->bind_param("ddi", $newLat, $newLng, $driverId);
                $updateStmt->execute();
                $updateStmt->close();
            }

            // 4. Lấy thông tin đánh giá (Rating)
            $ratingSql = "SELECT IFNULL(AVG(rating), 5) as avg_rating, COUNT(id) as total_reviews 
                         FROM reviews 
                         WHERE driver_id = ?";
            $ratingStmt = $conn->prepare($ratingSql);
            if ($ratingStmt) {
                $ratingStmt->bind_param("i", $driverId);
                $ratingStmt->execute();
                $ratingResult = $ratingStmt->get_result();
                $ratingData = $ratingResult->fetch_assoc();
                $ratingStmt->close();
            } else {
                $ratingData = ['avg_rating' => 5.0, 'total_reviews' => 0];
            }

            // 5. Chuẩn bị dữ liệu trả về
            $driverData = [
                'id' => $driverId,
                'full_name' => $row['full_name'] ?? 'Tài xế',
                'phone_number' => $row['phone_number'] ?? '',
                'current_lat' => (float)$newLat,
                'current_lng' => (float)$newLng,
                'avg_rating' => number_format((float)($ratingData['avg_rating'] ?? 5.0), 1),
                'total_reviews' => (int)($ratingData['total_reviews'] ?? 0)
            ];
            
            $drivers[] = $driverData;
        }
    }

    echo json_encode($drivers);
    
} catch (Exception $e) {
    http_response_code(500);
    echo json_encode([
        'error' => true,
        'message' => 'Lỗi khi lấy danh sách tài xế: ' . $e->getMessage()
    ]);
}

$conn->close();
