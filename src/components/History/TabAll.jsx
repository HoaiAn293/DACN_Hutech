...
const TabAll = () => {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost/DACS_Hutech/backend/get_orders.php?user_id=${user.id}`, {
      credentials: 'include',
      headers: {
      ...
      {orders.map((order) => (
        <div key={order.id} className="bg-white rounded-xl shadow-md overflow-hidden">
          ...
          <div className={`text-white px-6 py-4 flex justify-between items-center ${
            order.status === 'Chờ xác nhận' ? 'bg-[#1d4e89]' :
            order.status === 'Đang giao' ? 'bg-yellow-500' :
            order.status === 'Đã nhận' ? 'bg-green-600' :
            order.status === 'Hoàn tất' ? 'bg-green-600' :
            'bg-red-600'
          }`}>
